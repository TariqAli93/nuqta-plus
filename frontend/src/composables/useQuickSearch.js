...
export function useQuickSearch() {
  ...
  const performSearch = async () => {
    ...
    try {
      ...
      // Search products
      searchPromises.push(
        productStore.fetchProducts({ search: query.value, limit: 5 }).then((res) => {
      ...
      // Search customers
      searchPromises.push(
        customerStore.fetchCustomers({ search: query.value, limit: 5 }).then((res) => {
      ...
      // Search sales
      searchPromises.push(
        saleStore.fetchSales({ search: query.value, limit: 5 }).then((res) => {
